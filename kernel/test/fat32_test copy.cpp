...
bool FAT32Test::test_fat32_directory_operations() {
    ...
    if (!fat32Memory) {
        ...
        FSResult mountResult = fat32->mount("test");
        if (mountResult != FSResult::SUCCESS) {
            console.add_message("FAT32 mount failed for directory operations test", VGA_RED_ON_BLUE);
            testPassed = false;
        } else {
            // Get root directory
            VNode* rootNode = nullptr;
            ...
            if (rootResult != FSResult::SUCCESS || !rootNode) {
                ...
                console.add_message("Creating first file...", VGA_CYAN_ON_BLUE);
                FSResult createResult1 = rootNode->create_file("test.txt", FileType::REGULAR);
                if (createResult1 == FSResult::SUCCESS) {
                    console.add_message("First file creation successful", VGA_GREEN_ON_BLUE);
                    
                    // Test 2: Lookup the created file
                    ...
                    if (lookupResult == FSResult::SUCCESS && foundNode) {
                        ...
                        if (deleteResult == FSResult::SUCCESS) {
                            ...
                            // Test 4: Create second file
                            console.add_message("Creating second file...", VGA_CYAN_ON_BLUE);
                            ...
                            if (createResult2 == FSResult::SUCCESS) {
                                ...
                                // Test 5: Delete second file
                                console.add_message("About to delete second file...", VGA_CYAN_ON_BLUE);
                                FSResult deleteResult2 = rootNode->delete_file("test2.txt");
                                console.add_message("Second file delete operation completed", VGA_CYAN_ON_BLUE);
                                ...
                                console.add_message("Testing FAT32 object validity after second delete...", VGA_CYAN_ON_BLUE);
                            } else {
                ...
                console.add_message("Cleaning up root node resources...", VGA_CYAN_ON_BLUE);
                rootNode->~VNode();
                console.add_message("Freeing root node memory in directory test...", VGA_CYAN_ON_BLUE);
                memMgr.free_physical_page(rootNode);
                console.add_message("Root node cleanup complete in directory test", VGA_CYAN_ON_BLUE);
            }
        }
