...
void ScrollableConsole::add_message(const char* message, u16 color) {
    ...
    // Temporarily disable interrupt protection to debug console corruption
    // disable_interrupts();
    ...
    if (!active) {
        // Use simple refresh instead of internal version
        ...
        int startMsg = (totalLines > 24) ? totalLines - 24 : 0;
        for (int i = 0; i < 24 && (startMsg + i) < totalLines; i++) {
            int bufferIndex = (startMsg + i) % BUFFER_LINES;
            
            // Copy message to VGA buffer directly
            for (int col = 0; col < 80 && buffer[bufferIndex][col] != '\0'; col++) {
                vgaBuffer[i * 80 + col] = colors[bufferIndex] | buffer[bufferIndex][col];
            }
        }
    }
...
bool ScrollableConsole::handle_keyboard_input(u8 scanCode) {
    // Disable interrupts for keyboard input handling
    disable_interrupts();
    
    // Handle F1 key to toggle active mode
    if (scanCode == KEY_F1) {
        toggle_active_mode_internal();
        enable_interrupts();
        return true;
    }
...
void ScrollableConsole::refresh_display() {
    // Disable interrupts for external refresh calls
    disable_interrupts();
    refresh_display_internal();
    enable_interrupts();
}

void ScrollableConsole::refresh_display_internal() {
    // Internal refresh function (assumes interrupts already disabled)
...
void ScrollableConsole::toggle_active_mode_internal() {
    active = !active;
    
    if (active) {
        // Entering scroll mode - stay at current position
        add_message_internal(">>> SCROLL MODE ACTIVE <<<", VGA_GREEN_ON_BLUE);
    } else {
        // Exiting scroll mode - jump to bottom
        scroll_to_bottom();
        add_message_internal(">>> NORMAL MODE <<<", VGA_YELLOW_ON_BLUE);
    }
    
    refresh_display_internal();
}

void ScrollableConsole::add_message_internal(const char* message, u16 color) {
    if (!message) return;
    ...
    // Don't auto-refresh in internal version
}
